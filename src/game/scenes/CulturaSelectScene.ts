...
export class CulturaSelectScene extends Phaser.Scene {
  ...
  create(data: { caller: string; options: CulturaId[] }) {
    ...
    this.add.rectangle(panelX, panelY, panelW, panelH, 0x1a1a2e, 1)
      .setScrollFactor(0).setDepth(1001);
    this.add.rectangle(panelX, panelY, panelW, panelH, 0x4a3f6b, 0)
      .setScrollFactor(0).setDepth(1001)
      .setStrokeStyle(2, 0x8866cc);

    // Title
    this.add.text(panelX, panelY - panelH / 2 + 22, "CULTURA CORPORATIVA", {
      fontFamily: "monospace", fontSize: "14px", fontStyle: "bold",
      color: "#f2c14e", stroke: "#000000", strokeThickness: 3,
    }).setOrigin(0.5).setScrollFactor(0).setDepth(1002);

    // Subtitle
    this.add.text(panelX, panelY - panelH / 2 + 44, "Escolha um beneficio para esta run:", {
    ...
    data.options.forEach((id, i) => {
      ...
      this.add.text(cx, cardY - cardH / 2 - 16, keyLabels[i], {
        fontFamily: "monospace", fontSize: "11px", fontStyle: "bold",
        color: "#88ccff",
      }).setOrigin(0.5).setScrollFactor(0).setDepth(1002);

      // Card background
      const card = this.add.rectangle(cx, cardY, cardW, cardH, 0x0d0d1a, 1)
        .setScrollFactor(0).setDepth(1001)
      ...
      this.add.text(cx, cardY - 50, def.icon, {
        fontFamily: "monospace", fontSize: "24px", fontStyle: "bold",
        color: "#f2c14e",
      }).setOrigin(0.5).setScrollFactor(0).setDepth(1003);

      // Name
      this.add.text(cx, cardY - 18, def.name, {
      ...
      // Description
      this.add.text(cx, cardY + 30, def.description, {
