...
export class PreloadScene extends Phaser.Scene {
  ...
  create() {
    ...
    const LOGO_Y = 8;
    this.add.rectangle(LOGO_X + LOGO_W / 2, LOGO_Y + LOGO_H / 2, LOGO_W, LOGO_H, C_RED_BG)
      .setDepth(5);
    this.add.text(LOGO_X + LOGO_W / 2, LOGO_Y + 10, "RH SISTEMAS", {
      fontFamily: FONT, fontSize: "15px", fontStyle: "bold", color: C_WHITE,
    }).setOrigin(0.5, 0).setDepth(6);
    this.add.text(LOGO_X + LOGO_W / 2, LOGO_Y + 28, "LTDA.", {
      fontFamily: FONT, fontSize: "11px", color: "#ffdddd",
    }).setOrigin(0.5, 0).setDepth(6);
    this.add.text(LOGO_X + LOGO_W / 2, LOGO_Y + 40, "www.rh-sistemas.com.br", {
      fontFamily: FONT, fontSize: "9px", color: "#ffaaaa",
    }).setOrigin(0.5, 0).setDepth(6);

    // ── Blue top-left version bar ──
    this.add.rectangle(LOGO_W / 2 + 4, LOGO_Y + LOGO_H / 2, LOGO_W + 8, LOGO_H, C_BLUE_BG)
      .setDepth(5);
    this.add.text(8, LOGO_Y + 6, "VIDA-CLT BIOS v3.1", {
      fontFamily: FONT, fontSize: "12px", fontStyle: "bold", color: C_WHITE,
    }).setDepth(6);
    this.add.text(8, LOGO_Y + 22, "Copyright (C) 1994-2026", {
      fontFamily: FONT, fontSize: "10px", color: C_GRAY,
    }).setDepth(6);
    this.add.text(8, LOGO_Y + 35, "Recursos Humanos LTDA.", {
      fontFamily: FONT, fontSize: "10px", color: C_GRAY,
    }).setDepth(6);
  ...
  private postLine(text: string, color = C_WHITE, extraDelay = 0) {
    ...
    const t = this.add.text(this.leftX, y, text, {
  ...
  private blankLine() { this.textY += 8; }
  ...
  private startBarLoop() {
    ...
    const statusBg = this.add.rectangle(W / 2, barY + 8, W, 28, C_BLUE_BG)
      .setDepth(9);

    const barText = this.add.text(8, barY, "", {
      fontFamily: FONT, fontSize: "12px", color: C_WHITE,
    }).setDepth(10);
  ...
  private fadeToMenu() {
    ...
    this.tweens.add({
      targets: overlay, alpha: 1, duration: 700, ease: "Power2",
