...
export class Phase5Scene extends Phaser.Scene {
  ...
  private fx!: SanityFx;
  private hud!: Hud;
  private doorCeo!: Phaser.GameObjects.Image;
  ...
  create() {
    ...
    addPhaseBackground(this, "bg-diretoria", HUD_TOP_H, FLOOR_Y);
    ...
    this.buildPlatform(520, FLOOR_Y - 72, 4);
    this.buildPlatform(820, FLOOR_Y - 30, 6);
    this.buildPlatform(1120, FLOOR_Y - 72, 5);
    this.buildPlatform(1400, FLOOR_Y - 30, 6);
    this.buildPlatform(1700, FLOOR_Y - 72, 4);

    // Door to CEO (top floor, locked)
    this.doorCeo = this.add.image(LEVEL_WIDTH - 60, FLOOR_Y - 30, "tex-door").setTint(0xaa4400);
    this.doorLabel = this.add.text(LEVEL_WIDTH - 60, FLOOR_Y - 72, "DIRETOR\n[BLOQUEADO]", {
      fontFamily: "monospace", fontSize: "9px", color: "#884422", align: "center",
    }).setOrigin(0.5);
    ...
    [this.carimbadores, this.arquivos, this.baterias, this.drops].forEach(g =>
    ...
    contactDmg(this.arquivos,     (e) => (e as ArquivoAmbulante).contactDamage);
    contactDmg(this.baterias,     (e) => (e as BateriaSocial).contactDamage);
    ...
    this.physics.add.overlap(this.player, this.enemyProjectiles, (_p, pObj) => {
      ...
      this.player.takeDamage((p.getData("damage") as number) ?? 10, 3);
      p.destroy();
    });
    ...
    const inkDmgGroups: [Phaser.Physics.Arcade.Group, number][] = [
      [this.carimbadores, 4], [this.arquivos, 15], [this.baterias, 4],
    ];
    ...
    this.input.keyboard!.addKey(Phaser.Input.Keyboard.KeyCodes.ESC).on("down", () => {
      this.scene.pause();
      this.scene.launch("PauseScene", { caller: "Phase5Scene" });
    });
    this.interactKey = this.input.keyboard!.addKey(Phaser.Input.Keyboard.KeyCodes.E);
    const doorZone = this.add.zone(this.doorCeo.x, this.doorCeo.y, 40, 60);
  ...
  private spawnEnemies() {
    ...
    [400, 1000, 1700].forEach((x) => {
      ...
      e.target = this.player;
      e.onDeath = () => {
        // On death, nearby enemies get +10 HP boost
        const nearby = [...this.carimbadores.getChildren(), ...this.arquivos.getChildren(), ...this.baterias.getChildren()] as any[];
        nearby.forEach(en => {
          if (en.active && Phaser.Math.Distance.Between(en.x, en.y, e.x, e.y) < e.getAuraRange()) {
            en.hp = Math.max(en.hp - 0, en.hp); // just cancel the death buff — simplified
          }
        });
      };
      this.baterias.add(e);
  ...
  private checkAllDefeated() {
    ...
    if (remaining === 0) {
      ...
      this.doorCeo.clearTint();
      this.doorLabel.setText("DIRETOR").setColor("#f2c14e");
  ...
  private handleSpecial(type: string, fx: number, fy: number, facing: 1 | -1, def: any) {
    switch (type) {
      ...
      case "throw_weapon":
        ...
        break;
      case "emp_pulse": {
        [this.carimbadores, this.arquivos, this.baterias].forEach(g =>
  ...
  private resolveAttack(hb: Phaser.Geom.Rectangle, step: number) {
    ...
    hitGroup(this.carimbadores, 4,  (c) => c as CarimbadorAutomatico);
    hitGroup(this.arquivos,     15, (c) => c as ArquivoAmbulante);
    hitGroup(this.baterias,     4,  (c) => c as BateriaSocial);
  }
  ...
  update(time: number, delta: number) {
    ...
    const nearDoor = this.allDefeated &&
      Phaser.Math.Distance.Between(this.player.x, this.player.y, this.doorCeo.x, this.doorCeo.y) < 40;
