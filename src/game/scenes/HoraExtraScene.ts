...
export class HoraExtraScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Title
    this.add
      .text(GAME_WIDTH / 2, 28, "HORA EXTRA — MODO CALOR", {
        fontFamily: F,
        fontSize: "18px",
        fontStyle: "bold",
        color: TEXT_ACCENT,
    ...
    this.add
      .text(
        GAME_WIDTH / 2,
        54,
        "Aumentar o calor dificulta o jogo mas multiplica as recompensas.",
        {
          fontFamily: F,
          fontSize: "9px",
          color: TEXT_DIM,
    ...
    this.add
      .text(
        GAME_WIDTH / 2,
        72,
        `Loop atual: ${run.loopCount ?? 0}   Reconhecimento: ${run.reconhecimento.toLocaleString("pt-BR")}`,
        {
          fontFamily: F,
          fontSize: "9px",
          color: "#888888",
  ...
  private buildHeatList() {
    ...
    HEAT_LEVELS.forEach((level, i) => {
      ...
      // Level label
      card.add(
        this.add.text(-260, 8, `[${i}] ${level.label.toUpperCase()}`, {
          fontFamily: F,
          fontSize: "13px",
          fontStyle: i === this.selectedHeat ? "bold" : "normal",
      ...
      card.add(
        this.add.text(
          -260,
          26,
          `HP ×${level.hpMult.toFixed(2)}   VR ×${level.vrMult.toFixed(2)}   +${level.recoBonus} Reco/run`,
          {
            fontFamily: F,
            fontSize: "8px",
      ...
      // Click to select
      const hit = this.add
        .rectangle(0, rowH / 2 - 2, 720, rowH - 4, 0xffffff, 0)
      ...
      hit.on("pointerover", () => {
        if (i !== this.selectedHeat) bg.setAlpha(0.7);
      });
  ...
  private buildLegend() {
    ...
    this.add
      .text(
        GAME_WIDTH / 2,
        ly,
        "CALOR AFETA: HP dos inimigos · VR dropado · Bônus de Reconhecimento ao fim da run",
        {
          fontFamily: F,
          fontSize: "8px",
          color: TEXT_DIM,
      ...
      .setOrigin(0.5);
    this.add
      .text(GAME_WIDTH / 2, ly + 16, "↑↓ Selecionar   ENTER Confirmar   ESC Voltar", {
        fontFamily: F,
        fontSize: "8px",
        color: "#333344",
      })
