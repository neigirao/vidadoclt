...
export class RankingScene extends Phaser.Scene {
  ...
  async create() {
    ...
    this.add
      .text(GAME_WIDTH / 2, 14, "RANKING — TOP FUNCIONÁRIOS", {
  ...
  private drawTable(entries: ScoreEntry[]) {
    ...
    entries.forEach((e, i) => {
      ...
      this.add.text(cols.apelido, y, e.apelido.slice(0, 16), {
        fontFamily: "monospace",
        fontSize: "11px",
        color: col,
      });
      this.add.text(cols.recnh, y, e.reconhecimento.toLocaleString("pt-BR"), {
        fontFamily: "monospace",
        fontSize: "11px",
        color: col,
      });
      this.add.text(cols.fase, y, e.reached_phase, {
        fontFamily: "monospace",
        fontSize: "10px",
        color: DIM,
      });
      this.add.text(cols.seed, y, e.seed || "-", {
        fontFamily: "monospace",
        fontSize: "10px",
        color: DIM,
      });
      this.add.text(cols.loops, y, `${e.loop_count}x`, {
        fontFamily: "monospace",
        fontSize: "10px",
        color: DIM,
      });
