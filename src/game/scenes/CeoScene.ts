...
export class CeoScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add
      .rectangle(LEVEL_WIDTH / 2, GAME_HEIGHT / 2, LEVEL_WIDTH, GAME_HEIGHT, 0x000000, 0.3)
      .setDepth(-1);
    ...
    const weaponDef =
      WEAPONS[(run.weaponId ?? classDef.startWeapon) as WeaponId] ?? WEAPONS[classDef.startWeapon];
    ...
    this.boss.onDemissao = () => {
      ...
      const flash = this.add
        .rectangle(LEVEL_WIDTH / 2, GAME_HEIGHT / 2, LEVEL_WIDTH, GAME_HEIGHT, 0xff4400, 0.4)
        .setScrollFactor(0)
        .setDepth(900);
      this.tweens.add({
        targets: flash,
      ...
      const warning = this.add
        .text(GAME_WIDTH / 2, GAME_HEIGHT / 2 - 40, "DEMISSÃO EM MASSA!", {
    ...
    const title = this.add
      .text(GAME_WIDTH / 2, 110, "SALA DA COBERTURA — CONFRONTO FINAL", {
        fontFamily: "monospace",
        fontSize: "14px",
        color: "#eaeaea",
        stroke: "#000000",
  ...
  private handleSpecial(type: string, fx: number, fy: number, facing: 1 | -1, def: WeaponDef) {
    switch (type) {
      ...
      case "spray_knockback": {
        this.resolveAttack(
          new Phaser.Geom.Rectangle(facing > 0 ? fx : fx - 120, fy - 30, 120, 60),
          3,
        );
