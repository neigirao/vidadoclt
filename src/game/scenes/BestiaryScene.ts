...
export class BestiaryScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Header
    this.add.text(GAME_WIDTH / 2, 16, "BESTIARIO CORPORATIVO", {
      fontFamily: "monospace", fontSize: "16px", color: "#f2c14e",
    }).setOrigin(0.5).setDepth(20);
    this.add.text(GAME_WIDTH / 2, 36, `${found} / ${total} entidades identificadas`, {
    ...
    enemyIds.forEach((id, idx) => {
      ...
      this.container.add(this.add.text(x + CARD_W - 4, y + 4, `F${def.phase}`, {
        fontFamily: "monospace", fontSize: "8px", color: discovered ? hex : "#333344",
      }).setOrigin(1, 0));
      ...
      if (discovered) {
        ...
        this.container.add(this.add.text(x + 70, y + 8, "???", {
          fontFamily: "monospace", fontSize: "12px", fontStyle: "bold", color: "#222233",
        }));
        this.container.add(this.add.text(x + 70, y + 28, `Fase ${def.phase} — não encontrado`, {
          fontFamily: "monospace", fontSize: "7px", color: "#1a1a22",
        }));
        this.container.add(this.add.text(x + 4, y + CARD_H - 14, "Derrote para desbloquear", {
          fontFamily: "monospace", fontSize: "7px", color: "#1a1a22",
        }));
      }
    });
