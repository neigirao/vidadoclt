...
function parseScore(data: unknown): Score | null {
  ...
  if (Object.keys(v).sort().join() !== "apelido,character_class,loop_count,reached_phase,reconhecimento,seed") return null;
  if (typeof v.apelido !== "string" || v.apelido.length > 16 || !/^[\p{L}\p{N} _.-]{1,16}$/u.test(v.apelido)) return null;
  if (!Number.isInteger(v.reconhecimento) || (v.reconhecimento as number) < 0 || (v.reconhecimento as number) > 100000) return null;
  if (!Number.isInteger(v.loop_count) || (v.loop_count as number) < 0 || (v.loop_count as number) > 10000) return null;
  ...
  if (v.character_class !== null && (typeof v.character_class !== "string" || !classes.has(v.character_class))) return null;
  return v as Score;
}

async function hashSubject(ip: string, pepper: string): Promise<string> {
  const bytes = new TextEncoder();
  const key = await crypto.subtle.importKey("raw", bytes.encode(pepper), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (new TextEncoder().encode(body).length > 2048) return response(413, { ok: false, error: "size" });
  ...
  if (!ip || ip.length > 45 || !/^[0-9a-fA-F.:]+$/.test(ip)) return response(503, { ok: false, error: "network" });
  const admin = createClient(url, serviceKey, { auth: { persistSession: false } });
  const subject = await hashSubject(ip, pepper);
  const { data: allowed, error: rateError } = await admin.rpc("accept_score_rate_limit", { p_subject_hash: subject });
