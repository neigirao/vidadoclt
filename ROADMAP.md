...
- **Ramificação de rotas**: 🟢 _fundação + 1ª rota divergente DE VERDADE_ — `RouteSelectScene` após a Fase 1 (2A Comercial / 2B Atendimento → `run.route`) e após a Fase 2 (3A Produto / 3B Tecnologia → `run.route2`), cada uma com modificador de run. **Piloto entregue**: a Fase 2 agora diverge por `run.route` além do stat — fundo (`bg-comercial` vs `bg-atendimento`), título (CHÃO DE VENDAS vs REUNIÃO INFINITA), layout (aberto/vertical vs baias apertadas) e composição de inimigos (agressivo ranged vs atrito defensivo). Ambos passam no `LevelValidator`. Falta replicar o padrão nas outras bifurcações (3A/3B) — mesmo molde, agora provado.
