...
src/
  game/
    ...
    entities/
      ...
      Boss.ts                # GerenteMicrogestor + EmailProjectil (⚠ NÃO INSTANCIADO — ver abaixo)
      CeoBoss.ts             # CeoBoss
...
- ✅ **Acessibilidade — FEITO** (era ⬜): remap de teclas (CONFIGURAÇÕES → ⌨ CONTROLES, `Settings.keybinds`), modo daltônico p/ telegraphs (`colorBlindSafe` — amarelo→azul + forma `!!`/`!`), texto escalável (`uiTextScale` — dicas + toasts) e toggle de fotossensibilidade (`reduceSanityFx`) — tudo em `Settings.ts` (ver seção Controles/Acessibilidade acima).
...
- **Números de feel são TUNÁVEIS numa fonte única: `systems/Juice.ts`** (tabela `JUICE`). Squash & stretch (`land`/`jump`/`hit`), hit-stop e presets de shake vivem lá — CombatFx delega (não duplica magic numbers). Editar `Juice.ts` afeta o jogo todo; `bun juice:report` imprime o orçamento; `Juice.test.ts` trava faixas sãs. Squash & stretch: `CombatFx.landSquash`/`jumpStretch`/`hitSquash` (o `hitSquash` = recuo ao tomar dano, ligado no `Player.takeDamage`); o helper `squash()` multiplica a escala BASE (não quebra sprites já escalados) e garante retorno exato. **O squash é VISUAL, e isso é garantido no código**: no Arcade o corpo físico escala junto com o sprite, então achatar tirava o personagem do chão (a caixa encolhia ancorada no TOPO → o PÉ subia ~4px). Isso realimentava (sai do chão → cai → aterrissa → landSquash → sai de novo) e o corpo afundava no piso a cada ciclo até vazar pela faixa de 32px: o player atravessava o cenário e ia parar no limite inferior do mundo, sob o HUD, sem alcançar nenhum interativo — **na Copa isso trancava a run** (impossível bater o ponto). `squash()` reancora o corpo no `onUpdate` do tween (único instante em que a escala muda; fazer isso a cada frame briga com a separação do Arcade e afunda de outro jeito), mantendo tamanho e linha do pé constantes em px de MUNDO. `Juice.test.ts` trava isso.
...
- **Ferramentas DEV no menu (gated por `import.meta.env.DEV`)**: `TESTAR FASE`, `LAB SPRITES`, `LAB VFX` e `TELEMETRIA` são itens de DESENVOLVIMENTO no `MenuScene`, marcados com `dev: true` em `ALL_MENU_ITEMS` e **filtrados do build PUBLICADO** (`.filter(it => import.meta.env.DEV || !it.dev)` — vira `!it.dev` no build, então somem). `SpriteLabScene` e `VfxLabScene` também só entram no array de cenas do `config.ts` sob `import.meta.env.DEV` (spread condicional; a classe é tree-shaken do bundle publicado — só o literal de rota no `MenuScene` fica). No `bun dev` (e no smoke/audit, que rodam contra o dev server, `DEV === true`) tudo continua visível — é o loop rápido de validar arte/pular pra fase. **NÃO listar como problema em auditorias**: no jogo publicado essas ferramentas não aparecem nem são alcançáveis. O **painel de telemetria** (`/telemetria`) é rota React, então o item do menu sumir não bastava — a rota é gerada por arquivo e ia pro bundle com o `recharts` junto (~380KB). O painel virou `src/components/TelemetriaPanel.tsx`, carregado por `lazy(() => import(...))` atrás de `import.meta.env.DEV`: no build a constante vira `false`, o ternário morre por DCE e o chunk some (medido: 380KB → 604B, `recharts` fora do bundle). A rota segue existindo em produção, respondendo "painel indisponível".
- **Poda do deploy (`scripts/prune-deploy.mjs`, roda no `bun run build`)**: `public/assets/sprites/` tem ~3900 PNGs que são a **fonte** do atlas; o jogo carrega o `atlas.png` + 6 soltos (texturas que precisam ser próprias: tileSprite/partícula/projétil). O Vite copia `public/` inteiro, então a arte-fonte inteira ia pro deploy sem ser lida. O script apaga `<output>/assets/sprites/` preservando a lista `KEEP` — a pasta-fonte no repo fica **intacta** (dev, LAB e scripts de arte continuam iguais). Medido: `.output` 28MB → 11MB. A `KEEP` é **verificada por teste** (`PruneDeploy.test.ts`): varre `src/` atrás de `/assets/sprites/<x>.png` e reprova se alguém carregar um solto fora da lista — senão a poda apagaria uma textura viva e o jogo quebraria **só em produção**.
