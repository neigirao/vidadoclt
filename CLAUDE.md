...
- **`🤖 REFAZER COM IA (GEMINI)`** — redesenho por IA (`scripts/frame-gemini.mjs`, modelo `gemini-2.5-flash-image`). Passa o frame-alvo + vizinhos bons da família como **contexto de estilo/personagem/escala** → o modelo redesenha só o frame mantendo o design. Fluxo com **gate de aprovação**: gera uma PRÉVIA em `public/assets/.frame-preview/<frame>.png` (gitignored, **não** toca no frame real nem re-empacota) e o LAB abre um modal com **ANTES · DEPOIS estático · ANIMADO** (o DEPOIS rodando no loop do estado no lugar do frame-alvo → pega pulo de baseline/silhueta em movimento). No modal dá pra **escolher os vizinhos-referência** (thumbnails clicáveis — um vizinho ruim envenena o redesenho), **editar um HINT** (instrução extra pro prompt) e **🔄 GERAR DE NOVO** (IA é estocástica; itera sem fechar). `✅ APROVAR` copia a prévia por cima do frame real, re-empacota o atlas e faz **`git commit`** escopado (PNG-fonte + atlas), `✖ DESCARTAR` joga a prévia fora. Endpoints: `/__frame-fix` (mode=gemini → prévia; aceita `hint`/`refs`), `/__frame-approve` (atômico + rollback), `/__frame-discard`. Requer `GEMINI_API_KEY` (env ou `.env.local` gitignored — **nunca** commitada) + billing habilitado (free tier retorna 429 p/ imagem).
