...
- **`🤖 REFAZER COM IA (GEMINI)`** — redesenho por IA (`scripts/frame-gemini.mjs`, modelo `gemini-2.5-flash-image`). Passa o frame-alvo + até 3 vizinhos bons da família como **contexto de estilo/personagem/escala** → o modelo redesenha só o frame mantendo o design. Fluxo com **gate de aprovação**: gera uma PRÉVIA em `public/assets/.frame-preview/<frame>.png` (gitignored, **não** toca no frame real nem re-empacota) e o LAB abre um modal **ANTES/DEPOIS**; `✅ APROVAR` copia a prévia por cima do frame real, re-empacota o atlas e faz **`git commit`** automático (PNG-fonte + atlas), `✖ DESCARTAR` joga a prévia fora. Endpoints: `/__frame-fix` (mode=gemini → prévia), `/__frame-approve`, `/__frame-discard`. Requer `GEMINI_API_KEY` (env ou `.env.local` gitignored — **nunca** commitada) + billing habilitado (free tier retorna 429 p/ imagem).

**Aplicar CONSCIENTEMENTE**: o audit só sinaliza candidatos — rescale cego corrompe frame de FX/pose (ex.: `gerente-hurt2` é uma explosão de impacto, não o personagem), e a IA pode destoar do estilo (por isso o gate ANTES/DEPOIS). Confira sempre antes de aprovar.
