...
**Conserto de frame (botões `🔧` no LAB, dev-only)**: consertos DETERMINÍSTICOS que preservam o design (reusam os pixels, não é redesenho de IA) — `scripts/frame-fix.mjs <frame> <mode>`, via endpoint `/__frame-fix` (re-empacota o atlas). `rescale` reescala o personagem p/ a mediana de altura da família (conserta pulo de tamanho); `copy-nearest` copia o vizinho bom (conserta vazio/quebrado). **Aplicar CONSCIENTEMENTE**: o audit só sinaliza candidatos — rescale cego corrompe frame de FX/pose (ex.: `gerente-hurt2` é uma explosão de impacto, não o personagem). Redesenho criativo do zero (IA/Gemini) não está ligado — precisaria de backend + API key; o motor determinístico cobre os defeitos mecânicos que o audit acha.
