...
FAR = emissive("far", (0.05, 0.05, 0.10), 1.0)
MID = emissive("mid", (0.07, 0.06, 0.11), 1.0)
NEAR = emissive("near", (0.05, 0.04, 0.07), 1.0)
WIN_WARM = emissive("win_warm", (1.0, 0.72, 0.32), 6.0)
WIN_COLD = emissive("win_cold", (0.55, 0.72, 1.0), 4.0)

planes = [(-46, 15, 30, FAR, 0.55), (-30, 11, 22, MID, 0.75), (-18, 7, 14, NEAR, 0.95)]
for depth, count, maxh, mat_, wchance in planes:
    for i in range(count):
        ...
        cube("tower", (w, 2.2, h), (x, depth, h / 2 - 6.0), mat_)
        ...
        for r in range(rows):
            ...
            for c in range(int(w)):
                ...
                cube(
                    "win",
                    (0.34, 0.06, 0.30),
                    (x - w / 2 + 0.6 + c * 1.1, depth + 1.16, r * 1.5 - 5.4),
                    wm,
                )
...
bpy.ops.object.camera_add(location=(0, 14, 1.2), rotation=(D(88), 0, D(180)))
